 ...
 * 该类从相机获取的数据为AR渲染背景，它创建并持有提供给ARCore的填充相机图像的纹理。
 */
public class BackgroundRenderer {
   ...
   * EGLConfig)}.
   * 分配和初始化背景渲染器所需的OpenGL资源。该方法必须在OpenGL线程上调用，比如在onSurfaceCreated调用，
   *
   * @param context Needed to access shader source.
   */
  public void createOnGlThread(Context context) {
    // Generate the background texture.创建纹理对象，并设置环绕参数
    int[] textures = new int[1];
    GLES20.glGenTextures(1, textures, 0);
    //通过将textureId设置为全局变量，保存纹理索引，用于后续传递给session
  ...
  public void draw(Frame frame) {
    ...
    // to be drawn first.
    //无需测试或写入深度，屏幕四边形具有任意深度，并且预计将首先绘制。
